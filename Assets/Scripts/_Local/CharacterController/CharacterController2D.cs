...
public class CharacterController2D : MonoBehaviour
{
    ...
    public void CheckCapsuleEndCollisions(bool bottom = true)
    {
        ...
        {
            if ((i == 0 && !raycastLeft) || (i==2 && !raycastRight)) continue;
            ...
            if (bottom)
            {
                ...
                //m_GroundColliders[i] = count > 0 ? m_HitBuffer[0].collider : null;

                if (count > 0)
                {
                    IsGrounded = true;
                    m_GroundColliders[i] = m_HitBuffer[0].collider;
                }
                else
                {
                    m_GroundColliders[i] = null;
                }
            }
